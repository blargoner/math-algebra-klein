...
def orSearch(inverseIndex, query):
    ...
    ns = set()
    for w in query:
        if w in inverseIndex: ns.update(inverseIndex[w])
    return ns
...
def andSearch(inverseIndex, query):
    ...
    True
    """
    # Adopting the convention that an empty query returns an empty result, even
    # though logically it should return all document ids by vacuous implication
    ns = set()
    for (k,w) in enumerate(query):
        wns = inverseIndex[w] if w in inverseIndex else set()
        if k == 0:
            ns = wns.copy()
        else:
            ns.intersection_update(wns)
    return ns
