...
def avg_list(vs):
    return scale(1/len(vs), add_list(vs))

def max_key(d):
    mk, mv = '', float('-infinity')
    for k, v in d.items():
        if v > mv:
            mk, mv = k, v
    return mk
...
def create_voting_dict(strlist):
    ...
    """
    return { ws[0]:[ int(w) for w in ws[3:] ] for ws in [ s.split() for s in strlist ] }
...
def policy_comparisons(sen, sens, voting_dict):
    return { s:policy_compare(sen, s, voting_dict) for s in sens if s != sen }
...
def most_similar(sen, voting_dict):
    ...
    """
    return max_key(policy_comparisons(sen, voting_dict.keys(), voting_dict))
...
def least_similar(sen, voting_dict):
    ...
    Example:
        ...
        'b'
    """
    return min_key(policy_comparisons(sen, voting_dict.keys(), voting_dict))
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    Example:
        ...
        True
    """
    return avg(policy_comparisons(sen, sen_set, voting_dict).values())
...
def find_average_record(sen_set, voting_dict):
    ...
    Example: 
        ...
        [0.0, 1.0, 1.0]
    """
    return avg_list([ r for s, r in voting_dict.items() if s in sen_set ])

# give the vector as a list
average_Democrat_record = [
    -0.16279069767441862, -0.23255813953488372, 1.0, 0.8372093023255813, 0.9767441860465116,
    -0.13953488372093023, -0.9534883720930233, 0.813953488372093, 0.9767441860465116, 0.9767441860465116,
    0.9069767441860465, 0.7674418604651163, 0.6744186046511628, 0.9767441860465116, -0.5116279069767442,
    0.9302325581395349, 0.9534883720930233, 0.9767441860465116, -0.3953488372093023, 0.9767441860465116,
    1.0, 1.0, 1.0, 0.9534883720930233, -0.4883720930232558, 1.0, -0.32558139534883723, -0.06976744186046512,
    0.9767441860465116, 0.8604651162790697, 0.9767441860465116, 0.9767441860465116, 1.0, 1.0,
    0.9767441860465116, -0.3488372093023256, 0.9767441860465116, -0.4883720930232558, 0.23255813953488372,
    0.8837209302325582, 0.4418604651162791, 0.9069767441860465, -0.9069767441860465, 1.0, 0.9069767441860465,
    -0.3023255813953488
]
...
def bitter_rivals(voting_dict):
    ...
    Example: 
        ...
        True
    """
    # This is inefficient because the matrix is symmetric!
    return min_key({ (k,l):policy_compare(k,l,voting_dict) for k in voting_dict.keys() for l in voting_dict.keys() if l != k })
